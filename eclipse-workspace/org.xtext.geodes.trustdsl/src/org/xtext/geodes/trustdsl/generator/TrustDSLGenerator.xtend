...
//        sb.append("\n    consensusCompromiseBF = " + consensusCompromiseBF)
	...
	def static String computeEpistemicCumulativeBF(Collection<SBoolean> opinions) {
	    if (opinions === null || opinions.contains(null) || opinions.empty) {
	    ...
	    if (dogmatic.empty) {
	        ...
	        val totalWeight = dogmatic.map[o | o.relativeWeight].reduce[acc, w | acc + w]
	...
	def static String computeWeightedBF(Collection<SBoolean> opinions) {
	    if (opinions === null || opinions.contains(null) || opinions.empty) {
	        throw new IllegalArgumentException("WBF: Cannot average null opinions")
	    }
	
	    if (opinions.size == 1) {
	        return formatSBoolean(opinions.iterator.next)
	    }
	    ...
	    val dogmatic = opinions.filter[o | o.uncertainty == 0.0]
	
	    if (dogmatic.empty && opinions.exists[o | o.certainty > 0.0]) {
	        val productOfUncertainties = opinions.map[o | o.uncertainty].reduce[acc, u | acc * u]
	        val sumOfUncertainties = opinions.map[o | o.uncertainty].reduce[acc, u | acc + u]
	
	        var double numerator = 0.0
	        var double beliefAccumulator = 0.0
	        var double disbeliefAccumulator = 0.0
	        var double atomicityAccumulator = 0.0
	
	        for (o : opinions) {
	            val prod = productOfUncertainties / o.uncertainty
	        ...
	        numerator -= opinions.size * productOfUncertainties
	    ...
	    } else if (opinions.forall[o | o.uncertainty == 1.0]) {
	        resultBelief = 0.0
	        resultDisbelief = 0.0
	        resultUncertainty = 1.0
	
	        resultAtomicity = opinions.map[o | o.baseRate].reduce[acc, v | acc + v] / opinions.size
	    } else {
	        val totalWeight = dogmatic.map[o | o.relativeWeight].reduce[acc, w | acc + w]
	        ...
	        resultDisbelief = dogmatic.map[o | (o.relativeWeight / totalWeight) * o.disbelief].reduce[acc, v | acc + v]
	        resultUncertainty = 0.0
	        resultAtomicity = opinions.head.baseRate
	    }
		
	    return formatSBoolean(createSBoolean(resultBelief, resultDisbelief, resultUncertainty, resultAtomicity))
	}
	
	def static SBoolean uncertaintyMaximized(SBoolean sBoolean) {
	    val double p = sBoolean.projection
	
	    // Extreme cases
	    if ((sBoolean.baseRate == 1.0 && p == 1.0) || (sBoolean.baseRate == 1.0 && sBoolean.uncertainty == 1.0) || (sBoolean.baseRate == 0.0 && sBoolean.belief == 0.0)) {
	        return createSBoolean(0.0, 0.0, 1.0, sBoolean.baseRate)
	    }
	
	    // Normal cases
	    if (p < sBoolean.baseRate) {
	        return createSBoolean(0.0, 1.0 - (p / sBoolean.baseRate), p / sBoolean.baseRate, sBoolean.baseRate)
	    }
	
	    return createSBoolean((p - sBoolean.baseRate) / (1.0 - sBoolean.baseRate), 0.0, (1.0 - p) / (1.0 - sBoolean.baseRate), sBoolean.baseRate)
	}
	...
	def static double projection(SBoolean sBoolean) {
	    return adjust(sBoolean.belief + sBoolean.baseRate * sBoolean.uncertainty)
	}
	
	private def static double adjust(double value) {
	    return Math.round(value * 1000000.0) / 1000000.0
	}
	
	private def static double certainty(SBoolean sBoolean) {
        if (sBoolean.uncertainty == (0.0D / 0.0D)) return (0.0D / 0.0D);
        return 1.0D - sBoolean.uncertainty;
    }
}
